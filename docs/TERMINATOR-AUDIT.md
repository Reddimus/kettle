...
| `homogeneous_tabbar` (equal-width tabs) | config.py | ✅ cycle-620 + cycle-956 — `false` (kettle default) sizes per title length so the active/pressed visual stays bound to the visible label; `true` divides the strip evenly for users who want Terminator-style equal-width tabs; overflow falls back to homogeneous so a many-tab window never truncates | cycle-620 / 956 |
