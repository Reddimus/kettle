...
| `send_newline` | keybinds | ✅ `Action::SendNewline` writes a literal LF when explicitly bound or selected. The default Shift+Enter is forwarded distinctly to the client (xterm modifyOtherKeys without Kitty negotiation, CSI-u after negotiation); it is not a hidden Kettle keybinding. |
