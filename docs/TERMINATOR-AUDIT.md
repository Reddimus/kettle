...
| Drag a terminal to another position | paned.py drag-and-drop | A | Ships, keyboard and mouse. `move_split:{up,down,left,right}` rearranges from the keyboard (`Mux::move_pane_beside` -- lift the pane, collapsing whatever split it leaves, then graft it beside the target on the chosen side), reusing `goto_split`'s neighbour search so a pane lands where focus would have gone. The mouse gesture grabs a pane by its own titlebar: `pane_drop_zone` splits the pane under the cursor along its diagonals so every interior point names exactly one edge (a band-per-edge model leaves a dead middle, and on a narrow pane the bands overlap and the middle vanishes), `pane_drop_preview` gives the renderer the half a drop would take, and the release commits via `move_pane_beside`. Esc, a modal, or a focus loss abandons it. |
