...
|---|---|---|---|
| **Plugin system** | plugin.py + plugins/*.py | Status: in-progress. Event-hook foundation laid via cycle-324 Lua scripting + cycle-365 `kettle.on(event, cb)` registry; events shipped: `startup` / `tab_add` / `tab_close` / `bell` / `output` (cycles 365-377) + `pane_focus` (cycle-703 — emits `(prev_opt, cur)` for status-bar / activity-watch plugins). Per-plugin porting status: `activitywatch.py` → cycle-619 watcher; `custom_commands.py` → cycle-611 `menu-item =` config + cycle-375 `kettle.add_menu_item`; `terminalshot.py` → cycles 688/689; `logger.py` → cycle-621 `Action::ToggleSessionLog`; `urlhandlers.py` → cycle-X URL detection + cycle-695 `Action::ShowHelp`. Remaining: `launcher.py` (port to layout overlay), plus `title_changed` / `url_clicked` events for ergonomics. | continued — see CHANGELOG cycles 365-703 |
