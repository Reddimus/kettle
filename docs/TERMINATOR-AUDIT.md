...
| `ask_before_closing = always/multiple_terminals/never` | config.py | D | cycle-637 design in [`TERMINATOR-CONFIRM-DIALOG-DESIGN.md`](TERMINATOR-CONFIRM-DIALOG-DESIGN.md). Builds a generic `ConfirmDialogState` + `ConfirmAction` primitive that's reusable for future "kill process" / "discard layout" flows. `Action::CloseWindow` + `CloseTab` + `ClosePane` route through `maybe_confirm_then`. 8 sub-cycles. |
