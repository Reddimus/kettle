...
| `ask_before_closing = always/multiple_terminals/never` | config.py | A | cycles 637 + 638 + 648 + 652 + 660 + 662 — confirm-dialog 6/8 sub-cycles shipped: should_prompt helper, state types, keyboard nav state machine, renderer bottom-bar, CloseWindow/CloseTab/ClosePane interception. Mouse hit-test (sub-cycle 7) + audit-doc (sub-cycle 8) remain; the user-visible end-to-end behavior is live (run `kettle` with `ask-before-closing = always`, press CloseWindow → see modal). Cycle-661 deploy. |
