...
| `paste-images` (`paste-image`) | bool | `on` | When the OS clipboard holds a **bitmap** (a screenshot from Win+Shift+S, Snipping Tool, macOS Cmd+Shift+4, GNOME Screenshot) rather than a file or text, write it to a temporary PNG and paste that path — the same channel `paste-files` uses, so CLI agents read the image without relying on their own clipboard-bitmap decoding. Files use owner-only permissions inside a per-process scratch directory (per-user Local App Data on Windows; the OS temp directory elsewhere), are bounded in count and total size, and are **deleted when kettle exits** (a path in an old transcript will not resolve later). A directory left by a crashed run is reclaimed on next launch. `off` disables the branch. Separate from `paste-files` because this one writes captured screen content to disk |
...
the active backend, and whether fallback occurred. Screenshots honor the loaded
configuration through the same policy. The CI offscreen GPU self-test
deliberately uses `Config::default()` so a developer's machine configuration
