...
| `paste-image-preview` | bool | `on` | After the initiating pane accepts its own Kettle-created bitmap path, show a pane-local thumbnail receipt. It expands for four seconds, then contracts until its 30-second lifetime ends; hovering pauses expiry and expands it when the full card fits, and clicking opens the retained temporary PNG. The receipt says the path was pasted and waits for the client rather than claiming an attachment. Remote panes warn that the path is local; a short compact chip keeps that local-path label when the full warning cannot fit. `off` avoids creating or retaining preview pixels without disabling bitmap-to-path paste. Arbitrary paths are never previewed |
