...
| `completion-overlay` | `auto`\|`off` | `auto` | Present shell-owned completion candidates in a detached list at the top of the active pane, away from the command line. `auto` installs Kettle's completion binding only when the bundled Fish or PowerShell integration finds that shell's stock Tab binding; custom bindings remain untouched. Bash and Zsh expose cooperative `kettle_completion_show` / `kettle_completion_clear` helpers for existing completion widgets. Kettle only displays candidates; the shell still owns quoting, insertion, cycling, and execution. `off` exports the capability as disabled and leaves the shell's Tab behavior unchanged |
