...
| `modify-other-keys` (`modify_other_keys`) | `enter`\|`off` | `enter` | Controls only Kettle's modified-Enter fallback before an application queries or sets a keyboard protocol. `enter` preserves distinct Shift/Ctrl/Alt+Enter for non-negotiating CLI clients; `off` removes that fallback but still honors application-selected xterm levels and Kitty CSI-u. This is deliberately not an assumed xterm level: negotiated state always starts at zero, because assuming level two would also turn Ctrl+I into a non-Tab sequence for every legacy client. Reloads live for existing and future panes; Kitty CSI-u still takes precedence |
