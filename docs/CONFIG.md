...
| `gpu-force-software` | bool | `false` | Force wgpu's software/fallback adapter (slow; for debugging GPU-driver issues). **Applies on next launch.** Note: you rarely need this for resilience — kettle **auto-recovers** from a lost GPU device (a driver TDR/reset, a Remote Desktop console↔session transition, or VRAM exhaustion) by rebuilding the renderer on a backoff, escalating from your configured GPU → any other hardware adapter → the software rasterizer, all while the terminal panes keep running. When recovery lands on software, the window title shows a "software rendering (GPU unavailable)" notice |
