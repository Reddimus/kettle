...
| `paste-image-preview` | bool | `on` | After the initiating pane accepts its own Kettle-created bitmap path, show a pane-local thumbnail receipt. It expands for four seconds, then contracts until its 30-second lifetime ends. Hover pauses that timer, but a two-minute hard limit always removes the receipt. Click the body to open the retained PNG or `×` to dismiss it. The receipt says the path was pasted and waits for the client rather than claiming an attachment. Remote panes warn that the path is local. `off` avoids creating or retaining preview pixels without disabling bitmap-to-path paste. Arbitrary paths are never previewed |
