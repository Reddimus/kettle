...
| `modify-other-keys` (`modify_other_keys`) | `auto`\|`always`\|`off` | `auto` | Controls only Kettle's modified-Enter fallback before an application queries or sets a keyboard protocol. `auto` requires both noncanonical input and a foreground Unix/macOS job distinct from the shell (or a direct non-shell launch); this matters because zsh's own line editor is also noncanonical. On Windows it requires shell integration to report a running command or a direct non-shell launch. An ordinary or unknown shell prompt receives plain Enter, preventing the literal `;2;13~` suffix an unsolicited xterm sequence leaves behind. `always` preserves the former behavior for uninstrumented custom shells/SSH/WSL; legacy `enter` is accepted as its alias. `off` removes the fallback. All modes still honor application-selected xterm levels and Kitty CSI-u, which take precedence. Reloads live, and GUI, control-plane `send_keys`, and broadcast input evaluate the policy separately for each target pane |
