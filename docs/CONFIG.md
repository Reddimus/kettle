...
| `update-policy` | `off`\|`notify`\|`auto` | `notify` | Stable-channel behavior after the first-launch privacy skip: no automatic request, a once/day passive banner, or an authenticated background install used after the next restart. Official installer ownership is required for installation. See [UPDATES.md](UPDATES.md) |
