...
| `macos-option-as-alt` | `none`\|`left`\|`right`\|`both` | `none` | Selects which macOS Option key behaves as terminal Alt. `none` preserves normal macOS composition on both sides, so Option-produced symbols and accented characters reach the PTY without a Meta/ESC prefix. A selected side uses the unmodified key character and keeps `Alt` for keybinds, legacy xterm encoding, Kitty keyboard encoding, and modifier parameters. Ctrl+Option and Cmd+Option chords keep `Alt` in every mode, matching macOS's suppression of Option composition for those chords. Applies to existing windows on live reload. The key remains parseable but is reported inert on non-macOS platforms so one shared config works everywhere |
...
| `paste-images` (`paste-image`) | bool | `on` | When the OS clipboard holds a **bitmap** (a screenshot from Win+Shift+S, Snipping Tool, macOS Cmd+Shift+4, GNOME Screenshot) rather than a file or text, write it to a temporary PNG and paste that path — the same channel `paste-files` uses, so CLI agents read the image without relying on their own clipboard-bitmap decoding. The source RGBA buffer is capped at 256 MiB; at most 64 owner-only PNGs and 256 MiB of their **final encoded bytes in aggregate** remain live inside one owner-private process directory (per-user Local App Data on Windows; the OS temp directory elsewhere). The directory is held before screenshot bytes are written; Unix creates each PNG descriptor-relative, and Windows pins the directory name. Partial/over-budget encodes are deleted through their creating handle. Successful files retain their original identity handle and are **deleted when kettle exits** (a path in an old transcript will not resolve later). Startup launches bounded background cleanup (250 ms, 64 stale candidates, 32 removals): it reclaims a directory older than 24 hours only when its name has the exact creator/session grammar, that PID is definitively dead, and every child is a verified private single-link regular file named `0001.png` through `0064.png`; a live sibling is never removed because of age alone. `off` disables the branch. Separate from `paste-files` because this one writes captured screen content to disk |
| `paste-files` | bool | `on` | When the OS clipboard holds a file list (a file copied in Explorer/Finder) rather than text, paste its path(s) as shell-quoted text — the channel CLI agents (Claude Code, Codex) accept for any file type, including video. Paths are quoted for the focused pane's shell, and translated to `/mnt/…` when the pane runs WSL. `off` restores the old behavior (a copied file pastes nothing). Explicit drag-and-drop always pastes a path regardless of this key |
...
| `shell-integration` | bool | `on` | Automatically load Kettle's integration when **Windows Kettle chooses pwsh/powershell as its default shell**. The user's `$PROFILE` loads first; Kettle then wraps the prompt through a fixed ASCII `-NoExit -Command` bootstrap that decodes the bounded UTF-8 snippet. `cmd.exe` is left untouched because its process cwd tracks `cd`. Unix/macOS shells, WSL shells, and every explicit `command =` currently require the manual snippet from [SHELL-INTEGRATION.md](SHELL-INTEGRATION.md), even when this key is `on`. `off` disables the PowerShell injection too |
| `completion-overlay` | `auto`\|`off` | `auto` | Present shell-owned candidates in a detached lane above the active command, never at the cursor. If the prompt is too close to the top, no card is drawn. `auto` replaces only the stock Fish or PowerShell Tab bindings, never custom bindings, and stays off inside tmux/screen. Bash and Zsh provide cooperative helpers. The shell still owns matching and edits. Changes apply to new shells so an existing wrapper never becomes invisible mid-session |
