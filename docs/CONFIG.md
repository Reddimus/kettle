...
| `record-dir` | path | `<config-dir>/recordings` | Directory that `record = on` writes per-session traces into (collision-safe `kettle-session-*.cast` files, mode `0600`, dir `0700`). Ignored when recording is off or an explicit `--record PATH` is given |
...
| `shell-integration` | bool | `on` | Auto-load kettle's shell integration into the **default** shell so it reports its working directory (OSC 7) + prompt marks (OSC 133) with **zero `$PROFILE` setup** — this is what makes the tab track `cd` on Windows PowerShell (whose `Set-Location` doesn't update the OS process cwd). pwsh/powershell are launched `-NoExit -EncodedCommand <kettle.ps1>` (your `$PROFILE` still loads first; kettle only wraps the resulting prompt). cmd.exe is left untouched (its process cwd tracks `cd`). `off` launches the shell as-is — source the integration from `$PROFILE` yourself (`kettle --shell-integration powershell >> $PROFILE`). Only affects the default shell, not an explicit `command =` |
