...
| `modify-other-keys` (`modify_other_keys`) | `always`\|`auto`\|`off` | `always` | Controls xterm `modifyOtherKeys` handling for modified Enter. `always` assumes level 2 until the focused application explicitly selects level 0, 1, or 2, preserving Kettle's existing Shift/Ctrl/Alt+Enter sequences for CLI clients that never negotiate. `auto` starts at level 0 and emits the modified-Enter sequence only after the application requests level 2. `off` always sends `CR` for Enter chords, keeps the reported level at 0, and ignores enable requests. Level 1 excludes Return because it already has well-known behavior. Reloads live for existing and future panes; Kitty CSI-u negotiation still takes precedence |
...
| `focused-split-color` (`split-divider-color-focused`) | color | theme `palette[4]` | Border color for the *focused* pane — the "here am I" accent. While **broadcast mode** is on (`Super+G`, or `Ctrl+Shift+G` on Windows since `Win+G` opens Game Bar), this is temporarily overridden by theme `palette[3]` (yellow) to signal the active state; the configured color is restored when broadcast turns off |
