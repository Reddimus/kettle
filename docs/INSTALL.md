...
Every release from **v1.3.4** onward ships a `.sha256` sidecar (current latest: v1.35.0)
generated on the same CI runner as the artifact. Verify a tarball
before extracting it:

```sh
# Linux / WSL
curl -fLO https://github.com/Reddimus/kettle/releases/download/v1.35.0/kettle-linux-x86_64.tar.gz
curl -fLO https://github.com/Reddimus/kettle/releases/download/v1.35.0/kettle-linux-x86_64.tar.gz.sha256
...
```sh
cargo test --workspace      # 318+ tests incl. an offscreen GPU pipeline check
