...
def accent_components(image) -> list[int]:
    """Return 4-connected accent regions inside the icon face."""
    background = MODULE.DARK_FACE
    accent = MODULE.DARK_ACCENT
    remaining = set()
    inset = max(1, round(image.width * 80 / MODULE.CANVAS))
    for y in range(image.height):
        for x in range(image.width):
            if not (inset < x < image.width - inset and inset < y < image.height - inset):
                continue
...
class PillowIconTests(unittest.TestCase):
    ...
    def test_macos_keeps_legacy_icon_and_has_native_composer_source(self):
        ...
        light = self.root / "AppIcon.icon" / "Assets" / "kettle-light.svg"
        dark = self.root / "AppIcon.icon" / "Assets" / "kettle-dark.svg"
        legacy = self.root / "AppIcon.icon" / "Assets" / "kettle.svg"
        self.assertTrue(document.is_file())
        self.assertEqual(document.read_text(), MODULE.icon_composer_json_source())
        self.assertEqual(light.read_text(), MODULE.icon_composer_svg_source(False))
        self.assertEqual(dark.read_text(), MODULE.icon_composer_svg_source(True))
        self.assertEqual(legacy.read_text(), dark.read_text())
        self.assertNotIn('"image-name" : "kettle.svg"', document.read_text())
        self.assertIn('"solid"', document.read_text())
        self.assertIn(
            f'"solid" : "{MODULE.srgb_source(MODULE.LIGHT_ACCENT)}"',
            document.read_text(),
        )
        self.assertNotIn('"fill-specializations"', document.read_text())
        self.assertIn('"value" : "kettle-light.svg"', document.read_text())
        self.assertIn('"appearance" : "dark"', document.read_text())
        self.assertIn('"value" : "kettle-dark.svg"', document.read_text())
        ...
        self.assertIn('x="0" y="0"', light.read_text())

    def test_macos_artwork_uses_one_system_mask_and_a_parallel_inset_face(self):
        composer = MODULE.icon_composer_primitives(True)
        self.assertEqual(len(composer), 4)
        outer, face, *mark = composer
        self.assertIsInstance(outer, MODULE.RectPrimitive)
        self.assertIsInstance(face, MODULE.RectPrimitive)
        self.assertEqual(outer.box, (0, 0, MODULE.CANVAS, MODULE.CANVAS))
        self.assertEqual(outer.radius, 0)
        self.assertEqual(face.box, MODULE.MAC_FACE_BOX)
        inset = MODULE.MAC_FACE_BOX[0]
        self.assertGreaterEqual(inset, 40)
        self.assertEqual(MODULE.MAC_FACE_BOX[2], MODULE.CANVAS - inset)
        self.assertEqual(MODULE.MAC_FACE_RADIUS, inset)

        left, top, right, bottom = MODULE.primitive_bounds(tuple(mark))
        self.assertGreaterEqual(left, 100)
        self.assertGreaterEqual(top, 140)
        self.assertLessEqual(right, MODULE.CANVAS - 70)
        self.assertLessEqual(bottom, MODULE.CANVAS - 130)
        self.assertAlmostEqual((left + right) / 2, MODULE.CANVAS / 2, delta=8)

        artwork = MODULE.icon_composer_svg_source(True)
        self.assertNotIn("<text", artwork)
        self.assertEqual(artwork.count("<polyline"), 2)
        self.assertEqual(artwork.count("<rect"), 2)
        self.assertNotIn("stroke-opacity", artwork)
        self.assertTrue(all(primitive.stroke == MODULE.DARK_ACCENT for primitive in mark))
    ...
    def test_full_terminal_mark_is_two_components_in_the_24_pixel_artifact(self):
        icon = MODULE.render_icon_at_size(MODULE.render_master(), 24)
        self.assertEqual(len(MODULE.terminal_primitives()), 2)
        self.assertEqual(len(accent_components(icon)), 2)
    ...
    def test_light_preview_is_an_exact_color_inverse_of_dark_mode(self):
        ...
        face_sample = (MODULE.CANVAS * 2, MODULE.CANVAS)
        self.assertEqual(dark.getpixel(face_sample), MODULE.DARK_FACE)
        self.assertEqual(light.getpixel(face_sample), MODULE.LIGHT_FACE)
        ...
        self.assertEqual(
            ...
            {
                ((0, 0, 0, 0), (0, 0, 0, 0)),
                (MODULE.DARK_FACE, MODULE.DARK_ACCENT),
                (MODULE.DARK_ACCENT, MODULE.DARK_FACE),
            },
        )
    ...
    def test_svg_sources_are_generated_from_the_shared_geometry(self):
        ...
        composer = self.root / "AppIcon.icon" / "Assets" / "kettle-dark.svg"
        self.assertEqual(composer.read_text(), MODULE.icon_composer_svg_source(True))
        self.assertIn('x="0" y="0"', composer.read_text())

        mutant = self.root / "composer-mutant.svg"
        mutated = composer.read_text().replace(
            'points="128,172 228,256 128,340"',
            'points="129,172 228,256 128,340"',
            1,
        )
    ...
    def test_svg_generation_never_uses_platform_text_newlines(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for svg in [
                ...
                root / "AppIcon.icon" / "Assets" / "kettle-light.svg",
                root / "AppIcon.icon" / "Assets" / "kettle-dark.svg",
    ...
    def test_icon_composer_variants_are_exact_palette_swaps(self):
        dark = MODULE.icon_composer_primitives(True)
        light = MODULE.icon_composer_primitives(False)
        self.assertEqual(len(dark), len(light))
        for dark_primitive, light_primitive in zip(dark, light):
            if isinstance(dark_primitive, MODULE.RectPrimitive):
                self.assertEqual(dark_primitive.box, light_primitive.box)
                self.assertEqual(dark_primitive.radius, light_primitive.radius)
                expected = (
                    (MODULE.DARK_ACCENT, MODULE.LIGHT_ACCENT)
                    if dark_primitive is dark[0]
                    else (MODULE.DARK_FACE, MODULE.LIGHT_FACE)
                )
                self.assertEqual((dark_primitive.fill, light_primitive.fill), expected)
            else:
                self.assertEqual(dark_primitive.points, light_primitive.points)
                self.assertEqual(dark_primitive.stroke, MODULE.DARK_ACCENT)
                self.assertEqual(light_primitive.stroke, MODULE.LIGHT_ACCENT)
    ...
    def test_full_and_compact_marks_both_follow_the_live_accent(self):
        ...
        with patch.object(MODULE, "DARK_ACCENT", alternate):
            full = MODULE.terminal_primitives()
