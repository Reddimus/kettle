...
#
# NOTE (cycle 919): the rasters currently committed were generated by a
# Pillow reproduction of the SVG (the Windows dev host lacked rsvg-convert),
# so re-running this rsvg-convert path produces visually-equivalent but
# NOT byte-identical output (anti-aliasing + PNG compression differ; both
# are valid 8-bit/color RGBA). This script also does NOT regenerate the
# Windows `packaging/windows/kettle.ico` (multi-resolution; built
# separately). Either rasterizer is fine — just commit one consistently.
#
