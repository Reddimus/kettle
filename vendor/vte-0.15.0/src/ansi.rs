...
/// Maximum number of out-of-band events retained by one synchronized update.
pub const SYNC_MARKER_CAPACITY: usize = 256;
...
struct SyncState<T: Timeout> {
    ...
    /// Out-of-band events ordered between byte offsets in `buffer`.
...
impl<T: Timeout> Default for SyncState<T> {
    fn default() -> Self {
        Self {
            ...
            markers: Vec::with_capacity(SYNC_MARKER_CAPACITY),
...
impl<T: Timeout> Processor<T> {
    ...
    where
        H: Handler,
    {
        self.advance_with_sync_markers(handler, bytes, &mut |_, _| {});
    }

    /// Process PTY bytes and synchronously dispatch out-of-band markers.
    ///
    /// Markers are invoked only while a synchronized update is committed,
    /// between the exact surrounding byte offsets supplied to
    /// [`push_sync_marker`](Self::push_sync_marker).
    pub fn advance_with_sync_markers<H, F>(
        &mut self,
        handler: &mut H,
        bytes: &[u8],
        marker_handler: &mut F,
    ) where
        H: Handler,
        F: FnMut(&mut H, u64),
    {
        ...
        while processed != bytes.len() {
            if self.state.sync_state.timeout.pending_timeout() {
                processed += self.advance_sync(handler, &bytes[processed..], marker_handler);
            } else {
    ...
    where
        H: Handler,
    {
        self.stop_sync_with_markers(handler, &mut |_, _| {});
    }

    /// End a synchronized update and dispatch its out-of-band markers.
    pub fn stop_sync_with_markers<H, F>(&mut self, handler: &mut H, marker_handler: &mut F)
    where
        H: Handler,
        F: FnMut(&mut H, u64),
    {
        self.stop_sync_internal(handler, None, marker_handler);
    }

    /// Insert an unforgeable out-of-band event at the current synchronized
    /// byte offset.
    ///
    /// Returns `false` outside synchronized output or when the marker bound is
    /// exhausted. No byte sequence from the PTY can create one of these
    /// markers.
    pub fn push_sync_marker(&mut self, id: u64) -> bool {
        let sync = &mut self.state.sync_state;
        if !sync.timeout.pending_timeout() || sync.markers.len() == SYNC_MARKER_CAPACITY {
            return false;
        }
        sync.markers.push(SyncMarker { offset: sync.buffer.len(), id });
        true
    }
        ...
        F: FnMut(&mut H, u64),
    {
        ...
        let markers = mem::take(&mut self.state.sync_state.markers);
        let offset = bsu_offset.unwrap_or(buffer.len());
        {
            let mut performer = Performer::new(&mut self.state, handler);
            let mut byte_offset = 0;
            for marker in markers.iter().copied().take_while(|marker| marker.offset <= offset) {
                self.parser.advance(&mut performer, &buffer[byte_offset..marker.offset]);
                performer.handler.sync_marker(marker.id);
                marker_handler(performer.handler, marker.id);
                byte_offset = marker.offset;
            }
            self.parser.advance(&mut performer, &buffer[byte_offset..offset]);
        }
        ...
        match bsu_offset {
            ...
            Some(bsu_offset) => {
                ...
                self.state.sync_state.markers.extend(
                    markers.into_iter().filter(|marker| marker.offset > bsu_offset).map(|marker| {
                        SyncMarker { offset: marker.offset - bsu_offset, id: marker.id }
                    }),
                );
            },
            ...
            None => {
                ...
                self.state.sync_state.markers.clear();
            },
        }
    }
    ...
    #[cold]
    fn advance_sync<H, F>(&mut self, handler: &mut H, bytes: &[u8], marker_handler: &mut F) -> usize
    where
        H: Handler,
        F: FnMut(&mut H, u64),
    {
        ...
        if self.state.sync_state.buffer.len() + bytes.len() >= SYNC_BUFFER_SIZE - 1 {
            ...
            self.advance_sync_csi(handler, bytes.len(), marker_handler);
            bytes.len()
        }
    }

    /// Handle BSU/ESU CSI sequences during synchronized update.
    fn advance_sync_csi<H, F>(&mut self, handler: &mut H, new_bytes: usize, marker_handler: &mut F)
    where
        H: Handler,
        F: FnMut(&mut H, u64),
    {
        ...
        for index in memchr::memchr_iter(0x1B, search_buffer).rev() {
            ...
            if escape == BSU_CSI {
                ...
                self.stop_sync_internal(handler, bsu_offset, marker_handler);
                break;
            }
        }
    }
...
pub trait Handler {
    /// Commit one internal out-of-band event at its synchronized byte offset.
    ///
    /// PTY input cannot invoke this callback; only
    /// [`Processor::push_sync_marker`] can enqueue it.
    fn sync_marker(&mut self, _id: u64) {}
...
mod tests {
    ...
    impl Handler for OrderedHandler {
        fn input(&mut self, c: char) {
            self.events.push(OrderedEvent::Text(c));
        }
    ...
    #[test]
    fn sync_markers_replay_at_their_exact_byte_offsets() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = OrderedHandler::default();

        parser.advance(&mut handler, &BSU_CSI);
        parser.advance(&mut handler, b"a");
        assert!(parser.push_sync_marker(7));
        parser.advance(&mut handler, b"b");
        assert!(handler.events.is_empty());

        parser.advance_with_sync_markers(&mut handler, &ESU_CSI, &mut |handler, id| {
            handler.events.push(OrderedEvent::CallbackMarker(id));
        });
        ...
        assert_eq!(
            ...
            vec![
                OrderedEvent::Text('a'),
                OrderedEvent::HandlerMarker(7),
                OrderedEvent::CallbackMarker(7),
                OrderedEvent::Text('b'),
            ]
        );
    }
    ...
    #[test]
    fn sync_markers_remain_ordered_across_nested_update_boundaries() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = OrderedHandler::default();

        parser.advance(&mut handler, &BSU_CSI);
        parser.advance(&mut handler, b"a");
        assert!(parser.push_sync_marker(1));
        parser.advance_with_sync_markers(
            &mut handler,
            b"\x1b[?2026l\x1b[?2026hb",
            &mut |handler, id| handler.events.push(OrderedEvent::CallbackMarker(id)),
        );
        assert!(parser.push_sync_marker(2));
        parser.advance_with_sync_markers(&mut handler, b"c\x1b[?2026l", &mut |handler, id| {
            handler.events.push(OrderedEvent::CallbackMarker(id));
        });
        ...
        assert_eq!(
            ...
            vec![
                OrderedEvent::Text('a'),
                OrderedEvent::HandlerMarker(1),
                OrderedEvent::CallbackMarker(1),
                OrderedEvent::Text('b'),
                OrderedEvent::HandlerMarker(2),
                OrderedEvent::CallbackMarker(2),
                OrderedEvent::Text('c'),
            ]
        );
    }
    ...
    #[test]
    fn sync_marker_queue_is_bounded_and_unavailable_outside_sync() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = OrderedHandler::default();

        assert!(!parser.push_sync_marker(0));
        parser.advance(&mut handler, &BSU_CSI);
        for id in 0..SYNC_MARKER_CAPACITY as u64 {
            assert!(parser.push_sync_marker(id));
        }
        assert!(!parser.push_sync_marker(SYNC_MARKER_CAPACITY as u64));

        let mut callbacks = Vec::new();
        parser.stop_sync_with_markers(&mut handler, &mut |_, id| callbacks.push(id));
        assert_eq!(callbacks, (0..SYNC_MARKER_CAPACITY as u64).collect::<Vec<_>>());
        assert!(!parser.push_sync_marker(0));
    }
