...
pub trait MasterPty: Downcast + Send {
    ...
    ///
    /// The default preserves the normal writer contract. Windows ConPTY
    /// overrides this with `PIPE_NOWAIT` on Kettle's side of its byte pipe and
    /// normalizes Windows' transient zero-byte result to `WouldBlock`; the
    /// synchronous handle passed to `CreatePseudoConsole` is unchanged.
    fn take_nonblocking_writer(&self) -> Result<Box<dyn std::io::Write + Send>, Error> {
        self.take_writer()
    }
